...
import com.hypelist.presentation.extensions.moveItems
...
fun ItemMovePopUp(
    ...
    SettingsBar(title = LocalContext.current.getString(R.string.move_action), scaffoldState) {
    }
    ...
    LazyVerticalGrid( columns = GridCells.Fixed(2), Modifier.fillMaxHeight(0.5f)) {
        items(otherHypelists.size) {
            HypelistSelectionView(modifier = Modifier
                .width((scrollWidth / 2).dp)
                .aspectRatio(1.0f)
                .clickable {
                    hypelistViewModel.moveItems(hypelist, otherHypelists[it])
