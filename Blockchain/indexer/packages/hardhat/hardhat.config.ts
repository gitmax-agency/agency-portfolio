...
const config = {
  ...
  networks: {
    hardhat: {
      forking: {
        url: "https://eth-mainnet.alchemyapi.io/v2/IzS2TqDT99uZtuu0v6oqlBayabFxLq3l",
        // blockNumber: 14689484
       }
    },
    /*
    localhost: {
...
task("fundedwallet", "Create a wallet (pk) link and fund it with deployer?")
  ...
  .setAction(async (taskArgs, { network, ethers }) => {
    ...
    if (localDeployerMnemonic) {
      ...
      console.log("\n" + url + "/pk#" + privateKey + "\n");
      return (await ethers.provider.getSigner().sendTransaction(tx)).wait();
    }
  });
...
task(
  ...
  async (_, { ethers }) => {
    ...
    try {
      ...
      let n: keyof typeof config.networks;
      for (n in config.networks) {
        // console.log(config.networks[n],n)
        try {
          const network = config.networks[n];
          if ('url' in network) {
...
async function addr(ethers:any, addr:string|number) {
  let n: number = 0
...
// function send(signer:eth.Signer, txparams:eth.providers.TransactionRequest) {
//   return signer.sendTransaction(txparams, (resp: eth.providers.TransactionResponse) => {
//     if (error) {
//       debug(`Error: ${error}`);
//     }
...
task("send", "Send ETH")
  ...
  .setAction(async (taskArgs, { network, ethers }) => {
    ...
    debug(txRequest.gasPrice + " wei");
    debug(JSON.stringify(txRequest, null, 2));

    return (await fromSigner.sendTransaction(txRequest)).wait();
  });
