...
contract Fund is Ownable {
    ...
    event DepositedAsset(address assetAddress, uint256 amount, address user);
    event WithdrewAsset(address assetAddress, uint256 amount, address user);
    ...
    uint256 constant MAX_ASSETS = 50; // max assets per fund (a Balancer limitation)
    ...
    constructor(string memory newTicker, string memory newName, address[] memory addresses, uint256[] memory weights) {
        ...
        fundTicker = newTicker;
        fundToken = new FundToken(newName, fundTicker);
        ...
        for (uint256 i = 0; i < assetCount; i++) {
            ...
            console.log("EVENT: AddedFundAsset, assetAddress %s", addresses[i]);
            emit AddedFundAsset(addresses[i]);
        }
    ...
    // Deposit one of the fund's assets to the fund. 
    ...
    function depositAsset(address assetAddress, uint256 amount) public {
        ...
        require(isFound, "assetAddress must be one of the fund assets");

        assetToken.transferFrom(msg.sender, address(this), amount);
        _depositToBalancer(tokenAddresses, tokenBalances, false, true);

        console.log("EVENT: DepositedAsset, assetAddress %s, amount %s, user %s", assetAddress, amount, msg.sender);
        emit DepositedAsset(assetAddress, amount, msg.sender);
    }

    // Deposit native token by converting it into one of the fund assets via an exchange
    function depositNativeToken() payable public {
        ...
        console.log("EVENT: DepositedAsset, assetAddress %s, amount %s, user %s", exchangeTokenAddress, amountToken, msg.sender);
        emit DepositedAsset(exchangeTokenAddress, amountToken, msg.sender);
    }

    // Processes a withdraw in the amount of fundToken (not the assetToken). It does not need to be approved first.
    ...
    function withdrawAsset(address assetAddress, uint256 amount) public {
       console.log("Withdrawing asset %s for %s fund token", assetAddress, amount);
    ...
    // Processes a withdraw of a fraction of all assets in the amount of fundToken
    function withdrawMultipleAssets(uint256 amount) public {
        console.log("Withdrawing multiple assets for %s fund token", amount);
        ...
        for (uint256 i = 0; i < assetCount; i++) {
            ...
            if (0 < balanceChange) {
                console.log("EVENT: WithdrewAsset, assetAddress %s, amount %s, user %s", assetAddresses[i], balanceChange, msg.sender);
                emit WithdrewAsset(assetAddresses[i], balanceChange, msg.sender);
            }
        }
    }

    // Processes a withdraw in native token in the amount of fundToken (not the native token)
    // This will fail if there is not enough of the exchangeToken in the pool
    function withdrawNativeToken(uint256 amount) public {
        console.log("Withdrawing native token for %s fund token", amount);
    ...
    // All tokens need to be sent to the Fund contract first.
    function initializeFund() onlyOwner public {
    ...
    // Adds an asset to the fund. A new token must be sent to the fund contract prior to calling this.
    function addFundAsset(address assetAddress, uint256 assetWeight) onlyOwner public {
        ...
        for (i = 0; i < assetCount; i++) {
            require(assetAddress != assetAddresses[i], "Asset is already in the fund");
        ...
        IERC20(assetAddress).approve(address(this), MAX_UNIT256); 

        _createPool();
        _depositAllToBalancer(true, true, false);

        console.log("EVENT: AddedFundAsset, assetAddress %s", assetAddress);
        emit AddedFundAsset(assetAddress);
    }

    // Removes an asset from the fund. 
    // The removed asset will withdrawn and sent to the sender.
    // May need to send some other assets to the fund first, or there may be not enough fund token in the sender's wallet to be burned.
    function removeFundAsset(address assetAddress) onlyOwner public {
        ...
        require(i < assetCount, "The asset must be one of the fund assets");
        uint256 totalWeight = 0;
        ...
        console.log("Returning %s of %s to sender", assetToken.balanceOf(address(this)), assetAddress);
        assetToken.transferFrom(address(this), msg.sender, assetToken.balanceOf(address(this)));

        _createPool();
        _depositAllToBalancer(true, true, false);

        console.log("EVENT: RemovedFundAsset, assetAddress %s", assetAddress);
        emit RemovedFundAsset(assetAddress);
    }
    ...
    // Withdraw accumulated fees in different tokens, redeposit them to Balancer, and send a corresponding amount of fund token to the owner.
    // Note that management fees are counted in the total Balancer pool balance, so this method does not change the balances, only mints the fund token.
    ...
    function withdrawManagementFees() onlyOwner public {
        ...
        // Redeposit all fee tokens back to the pool and send the fund token to msg.sender
    ...
    // TEST: get fund token balance of an address
    function getFundTokenBalance(address account) public view returns (uint) {
        return fundToken.balanceOf(account);
    }
    */
    ...
    function _createPool() internal {
        ...
        poolAddress = poolFactory.create(fundTicker, fundTicker, tokens, tokenWeights, swapFeePercentage, address(this), true, MANAGEMENT_FEE);
    ...
    function _withdrawAsset(address assetAddress, uint256 amount, address receiver) private {
        ...
        require(assetTokenIndex != -1, "assetAddress must be one of the fund assets");

        _withdrawFromBalancer(amount, assetTokenIndex, true, receiver);
    
        console.log("EVENT: WithdrewAsset, assetAddress %s, amount %s, user %s", assetAddress, amount, msg.sender);
        emit WithdrewAsset(assetAddress, amount, msg.sender);
    }
    ...
    function _depositAllToBalancer(bool isNewPool, bool isMint, bool isEvent) internal {
        ...
        for (uint256 i = 0; i < assetCount; i++) {
            ...
            if (isEvent && 0 < tokenBalances[i]) {
                console.log("EVENT: DepositedAsset, assetAddress %s, amount %s, user %s", tokenAddresses[i], tokenBalances[i], msg.sender);
                emit DepositedAsset(tokenAddresses[i], tokenBalances[i], msg.sender);
            }
        }
    ...
    function _depositToBalancer(address[] memory tokenAddresses, uint256[] memory tokenBalances, bool isNewPool, bool isMint) internal {
        ...
        if (isMint) _adjustFundTokenUserBalance();
    }
    ...
    function _withdrawFromBalancer(uint256 amount, int256 assetTokenIndex, bool isBurn, address receiver) internal {
        ...
        if (isBurn) _adjustFundTokenUserBalance();
    }
    ...
    // This will fail if the user does not have enough fund token balance to burn
    function _adjustFundTokenUserBalance() internal {
        ...
        if (bptTokenBalance < bptTokenBalanceNew) {
            fundToken.Mint(msg.sender, bptTokenBalanceNew - bptTokenBalance);
        } else {
            fundToken.Burn(msg.sender, bptTokenBalance - bptTokenBalanceNew);
        }
